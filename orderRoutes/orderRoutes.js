...
router.get("/getOrdersByUserId/:user_id", (request, response)=>{
    db.query(`select orders.created_at, orders.id AS orderId, orders.total, orders.quantity, 
    ...
    inner join users on orders.user_id = users.id where orders.user_id = ? order by orders.created_at DESC`, [request.params.user_id], (err, results)=>{
        if(err) throw err;
        response.send(results);
    })
})

router.post("/getOrdersByUserIdFilter/:user_id", (request, response)=>{
    db.query(`select orders.created_at, orders.id AS orderId, orders.total, orders.quantity, 
    products.name, products.id AS product_id, products.price, products.product_img,
    user_payment.id, user_payment.payment_type, user_payment.account_number ,
    users.first_name, users.last_name
    ...
    inner join products on orders.product_id = products.id
    inner join user_payment on orders.user_payment_id = user_payment.id
    inner join users on orders.user_id = users.id where orders.user_id = ? AND date(orders.created_at) = ?`, [request.params.user_id, request.body.date], (err, results)=>{
        if(err) throw err;
        response.send(results);
        console.log(results)
    })
...
router.post("/addOrder", (request, response)=>{
    ...
    response.send("Order successfully added, Product quantity successfully updated, cart items successfully removed");
